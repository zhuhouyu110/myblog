...
$general_options = $nc_option['general'] ?? array();
...
//移除 WP_Head 无关紧要的代码
if ($general_options['remove_wp_head_useless'] ?? false) {
...
//禁用日志修订功能
if ($general_options['post_revision'] ?? false) {
...
if ($general_options['disable_trackbacks'] ?? false) {
    //彻底关闭 pingback
...
//禁用 XML-RPC 接口
if ($general_options['disable_xmlrpc'] ?? false) {
...
if ($general_options['gravatar_speedup'] ?? false) {
...
if ($general_options['disable_emoji'] ?? false) {
...
if ($general_options['emoji_cdn'] ?? false) {
...
if ($general_options['disable_wp_widgets'] ?? false) {
...
if ($general_options['disable_core_update'] ?? false) {
...
if ($general_options['disable_theme_update'] ?? false) {
...
if ($general_options['disable_plugin_update'] ?? false) {
