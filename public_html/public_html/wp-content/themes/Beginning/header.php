			...
			<header id="header">
				<div class="box-md">
					<?php if ( is_home() || Bing_mpanel( 'headings_tags_downgrade' ) ) : ?>
						<h1 class="logo">
							<a href="<?php echo esc_url( home_url('index.php') ) ?>" title="<?php echo esc_attr( sprintf( '%s | %s', get_bloginfo( 'name', 'display' ), get_bloginfo( 'description', 'display' ) ) ); ?>"><?php bloginfo( 'name' ); ?></a>
						</h1>
					<?php else : ?>
						<div class="logo">
							<a href="<?php echo esc_url( home_url('index.php') ) ?>" title="<?php echo esc_attr( sprintf( '%s | %s', get_bloginfo( 'name', 'display' ), get_bloginfo( 'description', 'display' ) ) ); ?>"><?php bloginfo( 'name' ); ?></a>
