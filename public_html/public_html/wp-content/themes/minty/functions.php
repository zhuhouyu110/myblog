...
function minty_scripts_styles() {
	...
	if ( get_option('minty_infinitescroll') > 0 )
		wp_enqueue_script( 'jquery-inview', get_template_directory_uri() . '/js/vendor/jquery.inview.min.js', array( 'jquery' ), false, true );

	if ( get_option('minty_nprogress') )
		wp_enqueue_script( 'nprogress', get_template_directory_uri() . '/js/vendor/nprogress.min.js', array( 'jquery' ), '0.1.3', true );

	if ( get_option('minty_lazyload') )
		wp_enqueue_script( 'jquery-lazyload', get_template_directory_uri() . '/js/vendor/jquery.lazyload.min.js', array( 'jquery' ), '1.9.3', true );

	if ( get_option('minty_snowfall') == 'site' || (get_option('minty_snowfall') == 'home' && is_home()) )
		wp_enqueue_script( 'snowfall', get_template_directory_uri() . '/js/vendor/snowfall.js', array(), '1.0', true );

	if ( get_post_gallery() ) {
    	wp_enqueue_script( 'jquery-fancybox', get_template_directory_uri() . '/js/vendor/fancybox/jquery.fancybox.pack.js', array( 'jquery' ), '2.1.5', true );
   		wp_enqueue_style( 'jquery-fancybox', get_template_directory_uri() . '/js/vendor/fancybox/jquery.fancybox.css', array(), '2.1.5' );
	}
	
	wp_enqueue_script( 'minty-script', get_template_directory_uri() . '/js/script.js', array( 'jquery' ), '2.1', true );
...
if ( function_exists(wp_pagenavi) ) {
	echo '<div class="navigation"><nav class="pagination" role="navigation">';
	wp_pagenavi();
	echo '</nav></div>';
} else {
 ...
 * Get Post Cover.
 */
function minty_get_post_cover($post_id=false) {
	if(!$post_id) $post_id = get_the_ID();
	if ( has_post_thumbnail() ) {
		return get_the_post_thumbnail( $post_id, 'post-thumbnail', array('itemprop' => 'image', 'lazyload' => 1) );
	} else {
...
function auto_redirect_after_logout(){   
  wp_redirect('http://zhuhouyu.xyz/blog/');   
  exit();   
}
/*  
*自定义登陆之后的重定向链接   
