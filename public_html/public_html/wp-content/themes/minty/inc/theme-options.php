...
$options = array(
    ...
    array(
        'name'  => '分享代码',
        'desc'  => '位于文章页的正文下方，不需要则留空。<a href="http://notepad.cc/share/AvXMNzE1ML" target="_blank">获取代码</a>',
...
function minty_add_theme_options_page() {
    ...
    add_theme_page('主题选项', '主题选项', 'edit_theme_options', basename(__FILE__) , 'minty_options_page');
}
...
<?php
global $currentTheme;
$updateData = @file_get_contents($currentTheme->get('AuthorURI') . '/api/minty.json');
$newVersionAvailable = false;
if ($updateData !== FALSE) {
    $updateData = json_decode($updateData);
    if ($updateData->version > $currentTheme->get('Version')) $newVersionAvailable = true;
};
...
echo '<a href="#panel_feedback" class="nav-tab">主题反馈</a>';
if ($newVersionAvailable) echo '<a href="#panel_update" class="nav-tab"><i></i>主题更新</a>';
?>
</h2>
...
<script>
var _hmt = [], _bdhmProtocol = (("https:" == document.location.protocol) ? " https://" : " http://");
...
<?php
foreach (array('Version', 'ThemeURI', 'Description', 'Author', 'AuthorURI') as $key=>$value) echo '_hmt.push(["_setCustomVar", ' . ($key+1) . ', "' . strtolower($value) . '", "' . $currentTheme->get($value) . '"]);';
?>
</script>
<?php
}
