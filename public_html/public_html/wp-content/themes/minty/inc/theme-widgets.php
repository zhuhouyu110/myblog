...
class Minty_PopularPosts extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$thumb_banner = isset($instance['thumb_banner']) ? $instance['thumb_banner'] : false;
		if ( ! $number )
 			$number = 7;
		...
		$output .= '<ul';
		if ($thumb) $output .= $thumb_banner ? ' class="minty_thumblist minty_thumblist_banner"' : ' class="minty_thumblist"';
		$output .= '>';
		if ( $popularposts ) {
			foreach ($popularposts as $post) {
				$output .= '<li><a href="' . get_permalink($post->id) . '" title="' . esc_attr($post->post_title) . '">';
				if ($thumb) $output .= $thumb_banner ? minty_get_post_cover($post->id) : minty_get_post_thumb($post->id);
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['thumb_banner'] = isset($new_instance['thumb_banner']) ? true : false;
	...
	function form( $instance ) {
		...
		$thumb_banner = isset($instance['thumb_banner']) ? $instance['thumb_banner'] : true;
		if ( $days < 7 || 365 < $days )
			$days = 30;
?>
...
?>
		...
		<p><input class="checkbox" type="checkbox" <?php checked($instance['thumb_banner'], true) ?> id="<?php echo $this->get_field_id('thumb_banner'); ?>" name="<?php echo $this->get_field_name('thumb_banner'); ?>" />
		<label for="<?php echo $this->get_field_id('thumb_banner'); ?>">横幅缩略图形式</label></p>
<?php
	}
...
?>
		...
		<ul class="minty_thumblist <?php if ($thumb_banner) echo 'minty_thumblist_banner'; ?>">
		<?php while ( $r->have_posts() ) : $r->the_post(); ?>
			<li>
				<a href="<?php the_permalink(); ?>">
					<?php echo $thumb_banner ? minty_get_post_cover($post->id) : minty_get_post_thumb($post->id); ?><span><?php get_the_title() ? the_title() : the_ID(); ?></span>
				</a>
			</li>
		<?php endwhile; ?>
		</ul>
